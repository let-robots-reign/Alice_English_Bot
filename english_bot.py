...
def display_start_message(response, storage):
    response.set_text("Здравствуй! Я бот, который помогает учить английский. Я могу переводить любые слова "
                      ...
                      "\"помощь\". Также вы можете все стереть и начать заново с помощью кнопки \"стереть данные\".\n"
                      "Давайте начнем!")
...
def display_rules(response, storage):
    response.set_text("Я умею переводить слова и фразы с русского языка на английский и наоборот и сохранять их "
                      "в ваш личный словарь. Чтобы лучше запомнить добавленные слова, вы можете тренировать слова "
                      "с помощью следующих тренировок:\n"
                      ...
                      "\nЯ могу быть полезен, если вам предстоить написать тестирование или контрольную, для которой "
                      "необходимо запомнить большое количество слов.")
    response = restart_dialogue(response)
    return response, storage
    ...
    if request.is_new_session:
        ...
        if "answer_awaiting" in user_storage:  # бот ожидает от пользователя какой-то определенный ответ
            ...
            return show_dict(response, user_storage)
                ...
                or request.command.lower() == "что ты умеешь?":
